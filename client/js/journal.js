...
function drawJournalList() {
  ...
  for (var key in journal_data) {
    ...
    html +=
      "<option value='" + key + "'>" + j.title + " (" + docs + ")</option>";
  }
...
function drawSelectedJournals() {
  for (var i = 0; i < selected_journals.length; i++) {
    if (
      $("#journal-filter li[journal-id='" + selected_journals[i] + "']")
        .length > 0
    ) {
      continue;
    }
    ...
    var html =
      "<li class='filter-item' journal-id='" +
      id +
...
function drawSelectedAuthors() {
  for (var i = 0; i < selected_authors.length; i++) {
    if (
      $("#author-filter li[author-id='" + selected_authors[i] + "']").length > 0
    ) {
      continue;
    }
    ...
    var html =
      "<li class='filter-item' author-id='" +
      id +
      "'><span class='filter-legend' \
      style='background-color:" +
      color_scale(id) +
...
function drawAuthorList() {
  ...
  for (var key in author_data) {
    ...
    var docs = j.count ? j.count : 0;
    html +=
      "<option value='" + key + "'>" + j.name + " (" + docs + ")</option>";
  }
